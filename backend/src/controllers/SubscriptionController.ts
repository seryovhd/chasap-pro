...
throw new AppError("ERRORX6: ID de empresa no válido", 400);
  }
  ...
  try {
    ...
    // Aquí puedes reactivar si necesitas guardar la suscripción
    /*
    ...
    */
    ...
    // Aquí puedes reactivar si necesitas actualizar el usuario
    /*
    const { id } = req.user;
    const userData = {};
    const userId = id;
  ...
  if (req.body.pix) {
    ...
    req.body.pix.forEach(async (pix: any) => {
      ...
      if (detahe.status === "CONCLUIDA") {
        ...
        if (!companyId || isNaN(Number(companyId))) {
          console.error("ERRORX7: ID de empresa no válido");
          return;
        }
