...
export class SocketCrypto {
  ...
  constructor(secretKey?: string, algorithm = 'aes-256-cbc') {
    try {
      ...
      logger.info('🔒 CRIPTOGRAFÍA DE SOCKETS INICIALIZADA');
    } catch (error) {
      logger.error(`❌ ERROR AL INICIALIZAR SISTEMA DE CRIPTOGRAFÍA: ${error.message}`);
      throw new Error('FALLO AL INICIALIZAR CRIPTOGRAFÍA');
    }
  }
