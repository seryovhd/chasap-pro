  ...
  io.on("connection", async socket => {
    ...
    socket.on("joinChatBox", async (ticketId: string) => {
      ...
      try {
        ...
        if (!ticket) {
          logger.warn(`INTENTO DE UNIRSE A UN CANAL DE TICKET INEXISTENTE - TICKET: ${ticketId} POR USUARIO: ${user.id}`);
          return;
        }
        
        if (ticket.companyId !== user.companyId) {
          logger.warn(`ACCESO DENEGADO PARA OTRA COMPAÑIA - TICKET: ${ticketId} POR USUARIO: ${user.id}`);
          return;
        }
        
        if (ticket.userId !== user.id && user.profile !== "admin") {
          logger.warn(`ACCESO DENEGADO PARA OTRO USUARIO - TICKET: ${ticketId} POR USUARIO: ${user.id}`);
          return;
        }
    ...
    socket.on("disconnect", async () => {
      logger.info(`USUARIO ${user.id} DESCONECTADO`);
      try {
        user.online = false;
        await user.save();
      } catch (error) {
