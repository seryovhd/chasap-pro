...
export default function AnnouncementsPopover({ visibleOnlyIfUnread = false }) {
  ...
  if (visibleOnlyIfUnread && unreadCount < 1) return null;
      ...
      >
        ...
        >
          <List component="nav" style={{ minWidth: 300 }}>
            {announcements.length > 0 ? announcements.map((item, key) => (
              <ListItem
                key={key}
                style={{
