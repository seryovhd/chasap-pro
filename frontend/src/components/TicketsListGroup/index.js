...
const TicketsListGroup = (props) => {
  ...
  useEffect(() => {
    const filteredTickets = tickets.filter((t) => {
      // Mostrar todos los tickets si showAll es true
      if (showAll) return true;

      // Si es usuario, solo mostrar los que están en sus colas o sin cola pero de grupo
      const isInUserQueue = queues.map((q) => q.id).includes(t.queueId);
      return isInUserQueue || t.isGroup === true;
    });

    dispatch({ type: 'LOAD_TICKETS', payload: filteredTickets });
  }, [tickets, status, searchParam, queues, profile, showAll]);
  ...
  if (selectedQueueIds.length > 0) {
    ticketsList = !!status
      ? ticketsList.filter(ticket =>
          user.profile === 'user'
            ? selectedQueueIds.includes(ticket?.queueId) || ticket?.isGroup === true
